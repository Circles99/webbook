...
func (s *ArticleMongoHandlerTestSuite) SetupSuite() {
	...
	hdl := InitArticleHandler(article.NewMongoDBDAO(s.mdb, node))
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				assert.True(t, publishedArt.Created > 0)
				assert.True(t, publishedArt.Updated > 0)
			},
			req: Article{
				Title:   "hello，你好",
				Content: "随便试试",
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				assert.Equal(t, int64(123), art.AuthorId)
				// 创建时间没变
				assert.Equal(t, int64(456), art.Created)
				// 更新时间变了
				assert.True(t, art.Updated > 234)
				var publishedArt article.PublishArticle
...

		{
			...
			after: func(t *testing.T) {
				...
				// 创建时间没变
				assert.Equal(t, int64(456), part.Created)
				// 更新时间变了
				assert.True(t, part.Updated > 234)
			},
			req: Article{
				Id:      3,
				Title:   "新的标题",
...

		{
			...
			after: func(t *testing.T) {
				...
				assert.Equal(t, int64(456), art.Created)
				assert.Equal(t, int64(234), art.Updated)
				...
				assert.Equal(t, int64(456), part.Created)
				// 更新时间变了
				assert.Equal(t, int64(234), part.Updated)
			},
			req: Article{
				Id:      4,
				Title:   "新的标题",
